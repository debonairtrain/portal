        ...
        $lga= mysqli_real_escape_string($con, $_POST['lga']);
          ...
          }else if($_POST['lga'] == '0')
          {
           ...
           else
           {
              ...
              $q = "UPDATE `applicant_profile` SET  `state_id`='$state',`lga_id`='$lga',
                 `first_name`='$first_name',`middle_name`='$middle_name',`last_name`='$last_name',`gender`='$gender',`religion`='$religion', `place_of_birth`='$pob',
                 `country_id`='$nationality', `day`='$day', `month`='$month', `year`='$year', `marital_status`='$marital_status'

                 WHERE applicant_id = '$applicant_id'"; //`programme_type_applied_for`='$programme_type',
