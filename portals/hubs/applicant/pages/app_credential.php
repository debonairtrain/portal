...
<?php if(is_applicant_admitted($con,$user_id)==true){
	echo "<span class='alert alert-danger col-md-10 ml-4 mt-3 mb-5 text-center'>You have been admitted, You can't update your Credentials......<span>";
}else {
	echo '<center><div id="saving_credential">
...
<?php
 echo get_applicant_credentials($con, $user_id)
...
$(document).ready(function(e){
		...
		$("#save_credential").submit(function(event){
			...
			var post_url = "hubs/applicant/php/update_credential"; //get form action url
...
//alert(token);
  ...
  var c=confirm("Are you sure you want to delete these item(s)?");
     if(c){
       $.post("hubs/applicant/php/delete_credential", {token:token},function(response, status){
         alert(response);
     window.setTimeout(link,2000);
       });
     }else{
